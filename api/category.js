...
module.exports = app => {
    ...
    const remove = async (req, res) => { 
        try {
            ...
            notExistOrError(subcategory, 'Categoria possuí subcategoria')

            const articles = await app.db('articles')
                .where({ categoryId: req.params.id })
            notExistOrError(articles, 'Categoria possuí articles')
    ...
    const withPath = categories => {
        const getPath = (categories, parentId) => {
    ...
    const getById = (req, res) => {
        ...
        const getTree = (req, res) => {
            app.db('categories')
                .then(category => res.json(toTree(withPath(category))))
                .catch(err => res.status(500).send(err))
        }
